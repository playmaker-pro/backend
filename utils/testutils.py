...
def get_verified_user_player():
    ...
    user = User.objects.create(email='usernameplayer', declared_role=definitions.PLAYER_SHORT)
...
def get_verified_user_coach():
    team = get_team()
    user = User.objects.create(email='usernamecoach', declared_role=definitions.COACH_SHORT)
    user.profile.team_object = team
    user.profile.VERIFICATION_FIELDS = ['bio']
    user.profile.COMPLETE_FIELDS = ['team_raw']  # , 'club_raw']
    user.profile.bio = 'Lubie Herbate'
...
def get_verified_user_club():
    club = get_club()
    user = User.objects.create(email='usernameclub', declared_role=definitions.CLUB_SHORT)
