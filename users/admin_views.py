...
class UserLoginHistoryView(View):
    ...
    def get(self, request, user_id):
        ...
        try:
            ...
            paginated_result = mongo_login_service.get_user_login_history_paginated(
                user_id, limit=per_page, offset=offset
            )

            # Get summary statistics (limit to avoid memory issues for users with huge history)
            summary_history = mongo_login_service.get_user_login_history(
                user_id, limit=1000
            )
            total_logins = sum(entry["login_count"] for entry in summary_history)
            current_streak = mongo_login_service.get_user_login_streak(user_id)
...
class BulkLoginHistoryExportView(View):
    ...
    def _export_csv(self, request, start_date_str: str, end_date_str: str):
        ...
        try:
            ...
            if initial_check["total_count"] == 0:
                messages.warning(
                    request,
                    f"No login data found for the period {start_date} to {end_date}.",
                )
                return self._render_form(request)

            # Use streaming response for large datasets
            return self._create_streaming_csv_response(
                mongo_login_service, start_date, end_date, initial_check["total_count"]
            )

        except ValueError as e:
            messages.error(
                request, f"Invalid date format. Please use YYYY-MM-DD format."
            )
            return self._render_form(request)
        except Exception as e:
    ...
    def _create_streaming_csv_response(
        self, mongo_service, start_date, end_date, total_count
    ):
        ...
        def csv_generator():
            ...
            while offset < total_count:
                ...
                if new_user_ids:
                    users = User.objects.filter(id__in=new_user_ids).only(
...
def export_user_login_history(request, user_id):
    ...
    try:
        login_history = mongo_login_service.get_user_login_history(user_id)

        # Create CSV response
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="login_history_{user.id}_{user.email}.csv"'
        )
        ...
        return render(
            request,
            "admin/users/user_login_history.html",
            {
                "user": user,
                "error": str(e),
                "title": f"Login History for {user.get_full_name() or user.email}",
