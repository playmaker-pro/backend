...
from . import models
from django.db.models import Q, Value, BooleanField, Case, When, ForeignKey, IntegerField, F
    ...
    def queryset(self, request, queryset):
        queryset = queryset.select_related('clubprofile', 'coachprofile', 'playerprofile').annotate(
            ...
            team_club_league_voivodeship_ver=Case(
                  When(declared_role="C", then=F("clubprofile__team_club_league_voivodeship_ver")),
                  When(declared_role="T", then=F("coachprofile__team_club_league_voivodeship_ver")),
                  When(declared_role="P", then=F("playerprofile__team_club_league_voivodeship_ver")),
               )
            )
...
class UserAdminPanel(UserAdmin):
    ...
    list_display = ('username', 'first_name', 'last_name', 'state', 'is_active', 'last_login', 'date_joined', 'get_profile', 'get_profile_permalink', linkify('profile'), 'get_profile_percentage', 'declared_role', 'get_data_mapper_id', 'get_team_object', 'get_team_club_league_voivodeship_ver')
    list_filter = ('state', 'declared_role', HasDataMapperIdFilter, VerificationInputTypeFilter)
