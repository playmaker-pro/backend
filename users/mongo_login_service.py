...
class MongoLoginService:
    ...
    def track_user_login(self, user_id: int) -> bool:
        ...
        try:
            ...
            daily_login = UserDailyLogin.objects(user_id=user_id, date=current_date).first()
            ...
            logger.error(f"Failed to track login for user {user_id}: {str(e)}", exc_info=True)
            return False
    
    def get_user_login_count(self, user_id: int, target_date: Optional[date] = None) -> int:
        ...
        try:
            ...
            daily_login = UserDailyLogin.objects(user_id=user_id, date=target_date).first()
    ...
    def get_user_login_history_paginated(
        ...
        try:
            ...
            logger.error(f"Failed to get paginated login history for user {user_id}: {str(e)}")
            return {
                'data': [],
                'total_count': 0,
                'limit': limit,
            ...
            while True:
                ...
                for streak_record in streak_records:
                    ...
                    streak_record.login_dates = [
                        login_date for login_date in streak_record.login_dates
    ...
    def get_all_users_login_history(
        ...
        try:
            ...
            # Get paginated results
            daily_logins = base_query.order_by('user_id', '-date').skip(offset).limit(limit)
