...
class UserDailyLogin(Document):
    ...
    login_count = IntField(default=1, min_value=1, help_text="Number of logins on this date")
    created_at = DateTimeField(default=utcnow, help_text="When this record was created")
    updated_at = DateTimeField(default=utcnow, help_text="When this record was last updated")
    ...
    @classmethod
    def increment_login_count(cls, user_id: int, login_date: date = None) -> 'UserDailyLogin':
        """
        ...
        try:
            ...
            if doc:
                ...
                doc.reload()
                logger.debug(f"Incremented login count for user {user_id} on {login_date}")
                return doc
            else:
                # Create new document
                doc = cls(user_id=user_id, date=login_date, login_count=1)
                doc.save()
                logger.debug(f"Created new login record for user {user_id} on {login_date}")
                return doc
                
        except Exception as e:
            logger.error(f"Failed to increment login count for user {user_id}: {str(e)}")
            raise
...
class UserLoginStreak(Document):
    ...
    current_streak = IntField(default=0, min_value=0, help_text="Current consecutive login streak")
    ...
    def add_login_date(self, login_date: date = None, login_timestamp: datetime = None) -> None:
        """
        ...
        try:
            ...
            self.save()
            logger.debug(f"Updated login streak for user {self.user_id}: current={self.current_streak}, max={self.max_streak}")
    ...
    @classmethod
    def update_user_streak(cls, user_id: int, login_date: date = None, login_timestamp: datetime = None) -> 'UserLoginStreak':
        """
