...
class TestMongoLoginService(TestCase):
    ...
    def test_track_user_login_database_error(self, mock_daily_login_class):
        ...
        # Mock database error
        mock_daily_login_class.increment_login_count.side_effect = Exception("Database error")
    ...
    def test_get_user_login_count_found(self, mock_daily_login_class):
        ...
        assert count == 5
        mock_daily_login_class.objects.assert_called_with(user_id=user_id, date=target_date)
    ...
    def test_get_user_login_history_paginated(self, mock_daily_login_class):
        ...
        mock_login2 = Mock(date=timezone.now().date() - timedelta(days=1), login_count=2)
        ...
        result = self.service.get_user_login_history_paginated(user_id, limit=2, offset=0)
