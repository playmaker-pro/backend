...
LEAGUES = [
    'Ekstraklasa', '1 Liga', '2 Liga',
    '3 Liga', '4 Liga', '5 Liga',
    'Klasa Okręgowa', 'A Klasa', 'B Klasa'
]
...
def get_club_without_img(clubs: Club, herbs: bool = False) -> Union[list, dict]:
    ...
    for club in clubs:
        for team in club.teams.all():
            if team.league and team.league.highest_parent and team.league.highest_parent.name in LEAGUES:
                if herbs:
                    herbs_list.append({
                        'name': club.name,
                        'picture': True if club.picture else None,
                        'club_id': club.id,
                        'voivodeship': club.voivodeship.name if club.voivodeship else None,
                        'team_name': [(team, team.league.highest_parent.name) for team in club.teams.all()],
                    })
                    break
    ...
    for club in clubs:
        ...
        if clubs_matched.get(club_id) and not club.picture:
            ...
            try:
                ...
                club.picture.save(file_name, File(open(PATH_TO_FLAGS + file_name, 'rb')))
