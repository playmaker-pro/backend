...
class FantasySettings(models.Model):
    excluded_leagues = models.TextField(
        help_text='Comma separated numbers eg. "20,21,23" ', null=True, blank=True
    )
    senior_leagues = models.TextField(
        help_text='Comma separated numbers eg. "20,21,23" ', null=True, blank=True
    )
    junior_leagues = models.TextField(
        help_text='Comma separated numbers eg. "20,21,23" ', null=True, blank=True
    )
...
class PlayerFantasyRank(models.Model):
    ...
    season = models.ForeignKey(
        Season, null=True, blank=True, on_delete=models.SET_NULL, db_index=True
    )
    ...
    def calculate(self):
        ...
        fantasy.calculate_fantasy_for_player(
            self.player.profile, self.season.name, self.senior
        )
...
class CalculateFantasyStats:
    ...
    def calculate_fantasy_for_player(
    ...
    def try_to_remove_fantasy_object(self, season, user, is_senior):
        ...
        try:
            pfr = PlayerFantasyRank.objects.get(
                season=season_object, player=user, senior=is_senior
            )
            pfr.delete()
        except PlayerFantasyRank.DoesNotExist:
            return

    def create_or_update_fantasy_object(
        self, season: str, points: int, games_count: int, user: User, is_senior
    ):
        ss = SeasonService()
        season_object = ss.get(season)
        PlayerFantasyRank.objects.update_or_create(
            ...
            player=user,
            senior=is_senior,
            defaults={"games_played": games_count, "score": points},
        )
