        ...
        for field, key in self.POSSIBLE_UNIQUE_FIELDS:
            ...
            if not lead:
                ...
                try:
                    ...
                    lead = self.get_object_or_none(LeadStatus, **{"first_name": first_name, "last_name": last_name})
...
class LeadStatusImport(CrmXmlImport):
    ...
    def _user_role(self, val):
        ...
        if role: self.obj["user_role"] = role
    ...
    def _team(self, val):
        if val:
            try:
                team_id = int(self.teams.loc[self.teams["Rozgrywki: klub"] == val]["Team_id"])
            except (ValueError, TypeError): return
            if team_id:
                fieldset = {"data_mapper_id": team_id}
                team = self.get_object_or_none(model=Team, **fieldset)
                if team: self.obj["team"] = team
    ...
    def import_data(self):
        ...
        for _, row in self.data_frame.iterrows():
            ...
            try:
                ...
                info_row = row["Dane"]
            if not(len(self.obj) and list(self.obj.keys()) == ["date_created", "created_by"]):
