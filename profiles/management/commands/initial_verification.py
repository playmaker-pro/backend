    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            try:
                ...
                logger.info('[!!!] User user: %s id: %i has no profile.' % (user, user.id))
                logger.error(e)
                continue
            if user.is_need_verfication_role and user.profile:
                if user.profile.verification is None:
                    user.profile.verification = ProfileVerificationStatus.create_initial(user)
                    user.profile.save()
                else:
                    if self.has_empty_verification_status_object(user.profile):
                        user.profile.verification.update_with_profile_data()
                        logger.info('Profile verification object updated with its defaults')
