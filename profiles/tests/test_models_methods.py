...
class ChangeRoleTests(TestCase):
    ...
    def test_changing_role_to_scout_from_unverifed_player_cause_user_to_be_auto_verified(self):
        ...
        assert self.user.is_missing_verification_data is True
        print(f'---->  before {self.user.state}')
        change = models.RoleChangeRequest.objects.create(user=self.user, new=definitions.SCOUT_SHORT)
    ...
    def test_changing_role_to_guest_from_unverifed_player_cause_user_to_be_auto_verified(self):
        assert self.user.is_verified is True
        self.user.profile.bio = None
        self.user.profile.save()
        assert self.user.is_verified is False
...
class InitalPlayerProfileCreationTests(TestCase):
    ...
    def test_player_profile_set__position__should_affect__fantasy_position(self):
        ...
        for number, result in self.profile.FANTASY_MAPPING.items():
            ...
            print(f'position:{self.profile.position_raw} ({number}, {result}) fantasy:{self.profile.position_fantasy}')
...
class ProfileVerificationExistingProfileWithReadyForVerificationTests(TestCase):
    ...
    def test_user_alter_one_of_verification_fields_should_statuses_should_stay_the_same(self):
    ...
    def test_clears_one_of_verification_fields_should_cause_status_change(self):
        ...
        self.user.profile.bio = None
        self.user.profile.save()
        assert self.user.is_missing_verification_data is True  # @todo think about altering this behavior
...
class ProfileUserIsVerifiedAndModifiesVerificationFields(TestCase):
    def setUp(self):
        ...
        self.user.state = User.STATE_ACCOUNT_VERIFIED  # @tood we cannot change NEW=>Acc Ver
