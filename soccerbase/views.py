...
class PlayersTable(TableView):
    ...
    def filter_queryset(self, queryset):
        ...
        if self.filter_vivo is not None:
            ...
            clauses = (Q(playerprofile__new_voivodeship__name=p) for p in vivos)
            query = reduce(operator.or_, clauses)

            players_with_no_vivo = queryset.filter(
                playerprofile__new_voivodeship__isnull=True
            )
            no_vivo_clause = (
                Q(playerprofile__team_object__club__new_voivodeship__name=p) for p in vivos
            )
        ...
        if self.filter_year_min is not None:
            ...
            queryset = queryset.filter(
                playerprofile__birth_date__year__gte=mindate.year
            )
    ...
    def get_queryset(self):
        return (
            User.objects.select_related("playerprofile")
            .filter(
                declared_role="P",
                state=User.STATE_ACCOUNT_VERIFIED,
                playerprofile__birth_date__lte=get_datetime_from_age(15),
            )
            .order_by(F("last_login").desc(nulls_last=True))
        )


class PlayerTalbeQuickFilter(
    generic.TemplateView,
    mixins.PaginateMixin,
    mixins.ViewModalLoadingMixin,
    mixins.ViewFilterMixin,
    ...
    def filter_queryset(self, queryset):
        ...
        if self.is_juniors is True:
            queryset = queryset.exclude(
                playerprofile__birth_date__year__lt=timezone.now().year - 20
            )
...
class TeamsTable(TableView):
    ...
    def filter_queryset(self, queryset):
        ...
        if self.filter_vivo is not None:
            vivos = [i for i in self.filter_vivo]
            clauses = (Q(club__new_voivodeship__name=p) for p in vivos)
            query = reduce(operator.or_, clauses)
            queryset = queryset.filter(query)
...
class CoachesTable(TableView):
    ...
    def get_queryset(self):
        return (
            User.objects.select_related("coachprofile", "coachprofile__team_object")
            .filter(declared_role="T", state=User.STATE_ACCOUNT_VERIFIED)
            .order_by(F("last_login").desc(nulls_last=True))
        )
    ...
    def filter_queryset(self, queryset):
        ...
        if self.filter_league is not None:
            queryset = queryset.filter(
                coachprofile__team_object__league__highest_parent__name__in=self.filter_league
            )

        if self.filter_vivo is not None:

            vivos = [i for i in self.filter_vivo]
            clauses = (Q(coachprofile__new_voivodeship__name=p) for p in vivos)
            query = reduce(operator.or_, clauses)

            coaches_with_no_vivo = queryset.filter(
                coachprofile__new_voivodeship__isnull=True
            )
            no_vivo_clause = (
                Q(coachprofile__team_object__club__new_voivodeship__name=p) for p in vivos
            )
            no_vivo_query = reduce(operator.or_, no_vivo_clause)
