...
class PlayersTable(TableView):
    ...
    def filter_queryset(self, queryset):
        ...
        if self.filter_league is not None:
            queryset = queryset.filter(playerprofile__team_object__league__top_parent__name__in=self.filter_league)
...
class TeamsTable(TableView):
    ...
    def filter_queryset(self, queryset):
        if self.filter_league is not None:
            queryset = queryset.filter(league__top_parent__in=self.filter_league)
...
class CoachesTable(TableView):
    ...
    def filter_queryset(self, queryset):
        ...
        if self.filter_league is not None:
            queryset = queryset.filter(coachprofile__team_object__league__top_parent__in=self.filter_league)
