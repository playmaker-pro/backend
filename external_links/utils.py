...
def create_or_update_player_external_links(obj: Union[
    ...
    first updates or creates the ExternalLinks instance that represents the profile's external links. If the instance is
    created, it is associated with the profile object. It then retrieves all LinkSource objects from the database and
    collects all links from the given profile object by iterating through its fields and adding those that end in "_url"
    and have a non-empty value. For each link, it creates or updates an ExternalLinksEntity instance with the
    corresponding LinkSource and adds it to the ExternalLinks instance.
    """
